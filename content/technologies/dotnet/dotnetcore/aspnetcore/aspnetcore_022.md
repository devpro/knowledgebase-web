---
title: "ASP.NET Core 2.2"
date: 2019-09-13T16:07:49+02:00
draft: false
---

## New features

- [What's new in ASP.NET Core 2.2](https://docs.microsoft.com/en-us/aspnet/core/release-notes/aspnetcore-2.2?view=aspnetcore-2.2), [Announcing ASP.NET Core 2.2, available today!](https://devblogs.microsoft.com/aspnet/asp-net-core-2-2-available-today/)
  - [Endpoint Routing](https://blogs.msdn.microsoft.com/webdev/2018/08/27/asp-net-core-2-2-0-preview1-endpoint-routing/)
  - [HTTP/2 in Kestrel](https://blogs.msdn.microsoft.com/webdev/2018/08/22/asp-net-core-2-2-0-preview1-http-2-in-kestrel/)
  - [Healthchecks](https://blogs.msdn.microsoft.com/webdev/2018/08/22/asp-net-core-2-2-0-preview1-healthcheck/)
    - [Health checks in ASP.NET Core](https://docs.microsoft.com/en-us/aspnet/core/host-and-deploy/health-checks?view=aspnetcore-2.2)
    - [HealthChecksSample](https://github.com/aspnet/AspNetCore.Docs/tree/master/aspnetcore/host-and-deploy/health-checks/samples/2.x/HealthChecksSample)
  - [Open API Analyzers & Conventions](https://blogs.msdn.microsoft.com/webdev/2018/08/23/asp-net-core-2-20-preview1-open-api-analyzers-conventions/)
- Scott Hanselman
  - [Useful ASP.NET Core 2.2 Features](https://www.hanselman.com/blog/UsefulASPNETCore22Features.aspx)
  - [How to set up ASP.NET Core 2.2 Health Checks with BeatPulse's AspNetCore.Diagnostics.HealthChecks](https://www.hanselman.com/blog/HowToSetUpASPNETCore22HealthChecksWithBeatPulsesAspNetCoreDiagnosticsHealthChecks.aspx)
  - [ASP.NET Core 2.2 Parameter Transformers for clean URL generation and slugs in Razor Pages or MVC](https://www.hanselman.com/blog/ASPNETCore22ParameterTransformersForCleanURLGenerationAndSlugsInRazorPagesOrMVC.aspx)

## Migration

- [Migrate from ASP.NET Core 2.1 to 2.2](https://docs.microsoft.com/en-us/aspnet/core/migration/21-to-22?view=aspnetcore-2.2&tabs=visual-studio)
